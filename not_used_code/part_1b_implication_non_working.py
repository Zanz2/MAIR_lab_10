...
class DialogState:
    ...
    class AskSecondaryPreference(BaseState): # system ask secondary preferences
        def __init__(self, history):
            ...
            #open_preferences = [cat for cat, pref in self.history.Secondary_preferences.items() if pref is None]
            #self.history.last_inquiry = rnd.choice(open_preferences)

        def generate_sentence(self):
            confirm = SystemUtterance.generate_combination(self.history.speech_acts[-1].parameters, "ASKSECONDARY")
            if confirm != "":
                confirm = f"Ok, {confirm}. "
            return f"{confirm}{SystemUtterance.ask_information(self.history.last_inquiry)}"
    ...
    class ExpressSecondaryPreference(BaseState):  #( get express Secondary preference user input here)
        ...
        def process_user_act(self, speech_act):
            # if you want any optional preferences (like restaurant not being busy, a long time to stay and being good for children) specify them now
            # i'd like it to not be busy, and i want a romantic restaurant
